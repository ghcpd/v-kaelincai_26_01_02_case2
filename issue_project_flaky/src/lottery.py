"""
Marketing Lottery System Module
...
FLAKY BEHAVIOR INTENTIONALLY PLANTED:
- Random number generation without fixed seed causes non-deterministic behavior
- Dictionary/Set iteration order may vary (in some scenarios)
- Same inputs can produce different outputs across test runs
"""
...
class LotterySystem:
    """
    Marketing lottery system with flaky behavior.
    
    BUGS INTENTIONALLY PLANTED:
    1. No random seed set - causes non-deterministic test results
    2. Using set for prize type iteration - order may vary
    """
    ...
    def __init__(self, prize_pool: Optional[PrizePool] = None):
        ...
        # Should have: random.seed(some_fixed_value) for deterministic testing
    ...
    def draw(self, user: User) -> tuple[PrizeType, str]:
        ...
        FLAKY BEHAVIOR:
        - Random number generation without seed causes different results
        - Each test run may produce different prize distributions
        """
        # Check eligibility
        ...
        # This causes non-deterministic behavior in tests
        rand_value = random.random()
        
        # Calculate adjusted probabilities based on user
        adjusted_probs = self._calculate_adjusted_probabilities(user)
        
        # Determine prize based on random value
        cumulative = 0.0
        
        # BUG: Using set which has undefined iteration order in older Python
        # Even in Python 3.7+, this adds to non-determinism
        prize_types_set = {PrizeType.FIRST, PrizeType.SECOND, PrizeType.THIRD, PrizeType.NONE}
        
        for prize_type in prize_types_set:
            cumulative += adjusted_probs.get(prize_type, 0)
            if rand_value < cumulative:
                # Try to consume from prize pool
                if prize_type != PrizeType.NONE:
                    if not self.prize_pool.consume(prize_type):
                        # Prize pool exhausted, try next tier
                        continue
    ...
    def _calculate_adjusted_probabilities(self, user: User) -> Dict[PrizeType, float]:
        ...
        FLAKY BEHAVIOR:
        - Dictionary iteration may contribute to non-determinism
        """
    ...
    def batch_draw(self, users: list[User]) -> Dict[str, PrizeType]:
        ...
        FLAKY BEHAVIOR:
        - Processing order may affect results due to prize pool depletion
        - Without fixed random seed, results vary across runs
        """
        results = {}
        
        # BUG: List iteration order is stable, but combined with random.random()
        # without seed, results are still non-deterministic
        for user in users:
