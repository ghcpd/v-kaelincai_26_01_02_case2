...
FIXED VERSION: Deterministic behavior with seed control for testing.
"""
...
class LotterySystem:
    """
    Marketing lottery system with deterministic behavior.
    
    FIXED: Uses instance-level random generator for seed control.
    """
    ...
    def __init__(self, prize_pool: Optional[PrizePool] = None, random_seed: Optional[int] = None):
        ...
        # FIXED: Use instance-level random generator for deterministic behavior
    ...
    def draw(self, user: User) -> tuple[PrizeType, str]:
        ...
        FIXED: Uses seeded random generator for deterministic results.
        """
        # Check eligibility
        ...
        # FIXED: Use instance random generator instead of module-level random
        ...
        # FIXED: Use list for deterministic iteration order
        prize_types_list = [PrizeType.FIRST, PrizeType.SECOND, PrizeType.THIRD, PrizeType.NONE]
        
        for prize_type in prize_types_list:
    ...
    def batch_draw(self, users: list[User]) -> Dict[str, PrizeType]:
        ...
        FIXED: Deterministic results when seeded.
        """
        results = {}
        
        # FIXED: Deterministic processing with seeded random
        for user in users:
...
def create_lottery(seed: Optional[int] = None) -> LotterySystem:
    ...
    """
    # FIXED: Actually use the seed parameter!
